...
# The distribution holds one build of Slang and it is a release build, so
# there is nothing to install under the debug prefix. A copy installed
# there anyway puts a second dylib of every one of these names beside the
# debug builds of other packages: a Debug link then resolves Slang from
# one prefix and its neighbours from the other, no ordering of the two
# directories satisfies both, and CMake reports a cycle and drops the
# runtime search path it was asked to write. The imported targets name
# the release library for a Debug configuration instead, below.
...
# The one build there is, named for every configuration a consumer may be
# in. Without this a Debug consumer reaches the release library through
# CMake's own fallback, which is the same file by a longer road and says
# nothing about why there is no debug one.
file(APPEND "${CURRENT_PACKAGES_DIR}/share/slang/slangConfig.cmake" [[

foreach(_slang_target slang::slang slang::gfx slang::slang-llvm
                      slang::slang-glslang slang::slang-glsl-module
                      slang::slangc slang::slangd slang::slangi
                      slang::slang-dispatcher)
  if(TARGET ${_slang_target})
    set_property(TARGET ${_slang_target} PROPERTY
